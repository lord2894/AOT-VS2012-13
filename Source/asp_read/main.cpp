...
int main(int argc, char ** argv)
{
	...
	try
	{
		...
		{	// reading
			...
			std::ifstream in("C:\\RML\\Dicts\\SrcBinDict\\ASP.txt");
			assert(in.is_open());
			while(true)
			{
				std::string l,r;
		...
		{ // writing
			std::ofstream bin_out("C:\\RML\\Dicts\\BinDict\\asp_dict.bin", std::ios::binary);
			assert(bin_out.is_open());
			if (!bin_out.is_open())
			{
				std::cerr << "Cannot open output file " << "C:\\RML\\Dicts\\BinDict\\asp_dict.bin" << std::endl;
				return  -1;
